...
class FlightOptimizer:
    ...
    def preprocess_combined_weights(self, weights):
        ...
        for u, v, data in self.graph.edges(data=True):
            data['combined_score'] = (
                ...
                time_w * data['time'] +
                co2_w * data['co2'] +
                layover_w * 1  # One layover per hop
            )
    ...
    def get_dijkstra_path(self, src, dst):
        """Return Dijkstra path and its score."""
    ...
    def get_bellman_ford_path(self, src, dst):
        """Return Bellman-Ford path and its score."""
        path = nx.bellman_ford_path(self.graph, src, dst, weight='combined_score')
        return path, self.score_path(path, self.current_weights)

    def visualize_paths(self, paths_info, output_file="multi_criteria_flight_paths.html"):
        if not paths_info:
        ...
        print(f"\nMap with paths saved as '{output_file}'")
...
def main():
    ...
    print("\nRate your preferences from 1 (least) to 10 (most important):")
    ...
    optimizer.current_weights = weights  # Store weights for class methods

    optimizer.preprocess_combined_weights(weights)
    all_paths_info = []

    try:
        # Get Dijkstra path & score
        d_path, d_score = optimizer.get_dijkstra_path(src, dst)
        print("\nDijkstra Path:", " -> ".join(d_path))
        print(f"  Score: {d_score[0]:.2f}, Cost: ₹{d_score[1]}, Time: {d_score[2]} min, CO₂: {d_score[3]} kg, Layovers: {d_score[4]}")
        ...
        # Get Bellman-Ford path & score
        b_path, b_score = optimizer.get_bellman_ford_path(src, dst)
        print("\nBellman-Ford Path:", " -> ".join(b_path))
        print(f"  Score: {b_score[0]:.2f}, Cost: ₹{b_score[1]}, Time: {b_score[2]} min, CO₂: {b_score[3]} kg, Layovers: {b_score[4]}")
        all_paths_info.append({
            "label": "Bellman-Ford",
            "path": b_path,
            "score": b_score[0],
        ...
        # Top 5 multi-criteria paths (shortest_simple_paths)
        print("\nTop Multi-Criteria Paths:")
        ...
        for i, path in enumerate(paths_gen):
            ...
            print(f"{label} Path {i+1}: {' -> '.join(path)}")
            print(f"  Score: {score_data[0]:.2f}, Cost: ₹{score_data[1]}, Time: {score_data[2]} min, CO₂: {score_data[3]} kg, Layovers: {score_data[4]}")
            top_paths.append({
                "label": label,
                "path": path,
                "score": score_data[0],
                ...
                "time": score_data[2],
                "co2": score_data[3],
                "layovers": score_data[4]
            })

        # Visualize all paths: top multi-criteria + Dijkstra + Bellman-Ford
        all_paths_info = top_paths + all_paths_info
        optimizer.visualize_paths(all_paths_info)

    except nx.NetworkXNoPath:
        print("No valid path found between source and destination.")
